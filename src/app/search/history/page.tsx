"use client";

import React, { useEffect, useState } from "react";
import Link from "next/link";
import { HistoryIcon } from "@/icons";
import Header from "@/components/layout/Header";
import Footer from "@/components/layout/Footer";
import ListCard from "@/components/ui/ListCard";
import RemoveButton from "@/components/ui/RemoveButton";
import { useSetAtom } from "jotai";
import { loadingAtom } from "@/lib/utils/atoms";
import { getBrowsingHistory } from "@/lib/utils/localstorage";
import type { ListCardItem } from "@/types";

export default function BrowsingHistoryPage() {
  const setIsLoading = useSetAtom(loadingAtom);
  const [browsingHistory, setBrowsingHistory] = useState<ListCardItem[]>([]);

  useEffect(() => {
    setIsLoading(true);
    const history = getBrowsingHistory();
    setBrowsingHistory(history);
    setIsLoading(false);
  }, [setIsLoading]);

  const handleClearHistory = () => {
    setBrowsingHistory([]);
  };

  return (
    <>
      <Header
        title="閲覧履歴"
        tools={<RemoveButton onHistoryClear={handleClearHistory} />}
      />

      <div className="p-4 space-y-6">
        {/* アイテム一覧 */}
        {browsingHistory.length > 0 ? (
          <ListCard cardItems={browsingHistory} pageName="history" />
        ) : (
          <div className="text-center py-12">
            <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-violet-100 flex items-center justify-center">
              <HistoryIcon className="text-violet-500 text-3xl" />
            </div>
            <h3 className="text-lg font-semibold text-stone-700 mb-4">
              閲覧履歴がありません
            </h3>
            <p className="text-stone-500 mb-6">
              レシピや食材を見ると、
              <br />
              こちらに履歴として記録されます
            </p>
            <Link
              href="/"
              className="inline-flex items-center px-6 py-3 rounded-2xl font-medium bg-violet-400 text-white hover:shadow-lg hover:scale-105 active:scale-95 transition-all"
            >
              ホームに戻る
            </Link>
          </div>
        )}
      </div>

      <Footer />
    </>
  );
}
